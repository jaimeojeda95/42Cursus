...
/*   Updated: 2025/08/21 20:53:45 by jaojeda-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

// Aqui voy a "pintar" la ventana
void ft_draw_map(t_game *game)
{
	...
	while (y < game->rows)
	{
		...
		{
			mlx_image_to_window(game->mlx, game->tx.floor, (x * TILE), (y * TILE));
			if (game->map[y][x] == '1')
				mlx_image_to_window(game->mlx, game->tx.wall, (x * TILE), (y * TILE));
			else if (game->map[y][x] == 'C')
				mlx_image_to_window(game->mlx, game->tx.collectable, (x * TILE), (y * TILE));
			else if (game->map[y][x] == 'E')
				mlx_image_to_window(game->mlx, game->tx.exit, (x * TILE), (y * TILE));
			else if (game->map[y][x] == 'P')
				mlx_image_to_window(game->mlx, game->tx.player, (x * TILE), (y * TILE));
			x++;
		}
		y++;
	}
